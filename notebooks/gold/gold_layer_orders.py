...
df_fact_new.groupBy("order_id").count().filter("count > 1").show()

# COMMAND ----------

df_dimcus.groupBy("dim_customer_id").count().filter("count > 1").show()

# COMMAND ----------

df_dimpro.groupBy("dim_product_id").count().filter("count > 1").show()
...
df.filter(df.order_id == "O00293").show()
...
if spark.catalog.tableExists("databricks_catalog.gold.FactOrders"):
    ...
    df_fact_new.write.format("delta")\
        .option("path", "abfss://gold@<storage-account-name>.dfs.core.windows.net/FactOrders")\
